...
export default class LightningDatatableWithFilters extends LightningElement {
    @api tableData;
    @api tableColumns;
    @api filterColumnsAPINames=["Industry","ShippingCountry", "BillingCountry", "Rating"];
    @api filterColumnLabels=["Industry","Shipping Country", "Billing Country", "Rating"];
    filterData = [];
    tableDataCopy;
    pillData = [];
...
updatePicklistValues() {
        ...
        console.log('this.filterData 40 ===> ' + JSON.stringify(this.filterData));
    }
changeHandler (event) {
...
getFieldOptions (fieldApiName) {
        ...
        this.tableData.forEach((currentElement, index) => {
            if (tempValueArray.indexOf(currentElement[fieldApiName]) < 0) {
                ...
                if(currentElement[fieldApiName]!==undefined){
                    toReturn = [...toReturn, {
